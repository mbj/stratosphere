...
data QueuePolicy
  = QueuePolicy {policyDocument :: JSON.Object,
                 queues :: (ValueList (Value Prelude.Text))}
mkQueuePolicy ::
  JSON.Object -> ValueList (Value Prelude.Text) -> QueuePolicy
...
instance Property "Queues" QueuePolicy where
  type PropertyType "Queues" QueuePolicy = ValueList (Value Prelude.Text)

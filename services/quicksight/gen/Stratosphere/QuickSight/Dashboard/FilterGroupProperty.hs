module Stratosphere.QuickSight.Dashboard.FilterGroupProperty (
        module Exports, FilterGroupProperty(..), mkFilterGroupProperty
    ) where
import qualified Data.Aeson as JSON
import qualified Stratosphere.Prelude as Prelude
import Stratosphere.Property
import {-# SOURCE #-} Stratosphere.QuickSight.Dashboard.FilterProperty as Exports
import {-# SOURCE #-} Stratosphere.QuickSight.Dashboard.FilterScopeConfigurationProperty as Exports
...
data FilterGroupProperty
  = -- | See: <http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-quicksight-dashboard-filtergroup.html>
    FilterGroupProperty {haddock_workaround_ :: (),
...
mkFilterGroupProperty
  ...
  scopeConfiguration
  = FilterGroupProperty
      {haddock_workaround_ = (), crossDataset = crossDataset,
       filterGroupId = filterGroupId, filters = filters,
       scopeConfiguration = scopeConfiguration, status = Prelude.Nothing}
instance ToResourceProperties FilterGroupProperty where
  toResourceProperties FilterGroupProperty {..}
    = ResourceProperties
        {awsType = "AWS::QuickSight::Dashboard.FilterGroup",
